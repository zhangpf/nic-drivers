...
pub(crate) fn wait_clear_reg32(base: *mut u8, reg: u32, mask: u32) {
    ...
    while {
        value = unsafe { std::ptr::read_volatile(addr) };
        (value & mask) != 0
    } {
        println!(
            "waiting for flags {:x} in register {:x} to clear, current value {:x}",
            mask, reg, value
        );
...
pub(crate) fn wait_set_reg32(base: *mut u8, reg: u32, mask: u32) {
    ...
    while {
        ...
        println!(
            "waiting for flags {:x} in register {:x}, current value {:x}",
            mask, reg, value
        );
        sleep(Duration::from_millis(100));
    }
