...
use super::{Buffer, Stats, get_reg32, set_reg32, wait_clear_reg32,
    wait_set_reg32, set_flags32, clear_flags32};
...
impl DeviceInfo {
    pub fn new(pci_addr: &str, num_rx_queues: u32, num_tx_queues: u32) -> DeviceInfo {
        if num_rx_queues > MAX_QUEUES {
            panic!(format!("cannot configur {} rx queues: limit is {}", num_rx_queues, MAX_QUEUES))
        }
        if num_tx_queues > MAX_QUEUES {
            panic!(format!("cannot configur {} tx queues: limit is {}", num_tx_queues, MAX_QUEUES))
        }
    ...
    fn init_device(addr: *mut u8, num_rx_queues: u32, num_tx_queues: u32) -> (Vec<RxQueue>, Vec<TxQueue>) {
    ...
    fn start_rx_queue(&mut self, queue_id: u32) {
        ...
        for j in 0..queue.num_entries {
            ...
            let mut buffer = queue.mempool.borrow_mut().alloc_buf().expect("failed to allocate rx descriptor");
            unsafe {
        ...
        set_reg32(self.addr, IXGBE_RDT!(queue_id), queue.num_entries as u32 - 1);
    ...
    fn init_rx(addr: *mut u8, num_rx_queues: u32) -> Vec<RxQueue> {
        ...
        for i in 0..num_rx_queues {
            ...
            set_reg32(addr, IXGBE_SRRCTL!(i), 
                (get_reg32(addr, IXGBE_SRRCTL!(i)) & !IXGBE_SRRCTL_DESCTYPE_MASK) | IXGBE_SRRCTL_DESCTYPE_ADV_ONEBUF);
    ...
    pub fn read_stats(addr: *const u8, stats: Option<&mut Stats>) {
    	...
    	let rx_bytes = get_reg32(addr, IXGBE_GORCL) as u64 + ((get_reg32(addr, IXGBE_GORCH) as u64) << 32);
	    let tx_bytes = get_reg32(addr, IXGBE_GOTCL) as u64 + ((get_reg32(addr, IXGBE_GOTCH) as u64) << 32);
            ...
            match status & IXGBE_RXDADV_STAT_DD {
                ...
                _ => {
			        ...
			        unsafe {
                        ...
                        desc_cloned.read.pkt_addr = (*new_buf).buf_addr_phys + mem::size_of::<Buffer>();
