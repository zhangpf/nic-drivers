...
const PKT_DATA: [u8; 45] = [
	...
	'i' as u8, 'x' as u8, 'y' as u8                       // payload
...
fn main() {
    ...
    let mut mempool = ixgbe::Mempool::new(NUM_BUFS as u16, ENTRY_SIZE);
	{
		...
		for _ in 0..NUM_BUFS {
			...
			unsafe {
				...
				*chk_ptr = calc_ip_checksum(
					slice::from_raw_parts((data_offset as usize + 14) as *const u8, 20)
				);
			}
			bufs.push(buf);
		}
    ...
    loop {
		...
		for i in 0..BATCH_SIZE {
			unsafe {
				let seq_ptr = ((*bufs[i as usize]).data_offset() as usize + PKT_SIZE as usize - 4) as *mut u32;
				*seq_ptr = seq_num;
			}
			seq_num += 1;
		}
		...
		if (counter & 0xfff) == 0 {
			...
			if (t.duration_since(last_stats_printed)) > Duration::new(1, 0) {
				stats.update(&device);
				ixgbe::print_stats_diff(stats, old_stats, t.duration_since(last_stats_printed).as_nanos() as u64);
				old_stats = stats;
